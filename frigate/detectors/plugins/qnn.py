...
class QnnDetector(DetectionApi):
    ...
    def __init__(self, detector_config: QnnDetectorConfig):
        ...
        # detector subprocess restarts.
        atexit.register(self._release_context)
        logger.info(
    ...
    def _release_context(self) -> None:
        # Drop our reference before interpreter shutdown starts tearing down
        # C++ globals, so the QNNContext (and its fastrpc session) is released
        # while the runtime can still do so cleanly. atexit holds a reference
        # to self for the life of the process; the detector does too, so this
        # costs nothing.
        self._ctx = None
    ...
    def _decode(self, outputs: list[np.ndarray]) -> np.ndarray:
        ...
        # Second NMS pass, deliberately. The AI Hub yolov8_det export runs NMS
        # in-graph (QAIRT's per-inference timing names a 'yolo Nms' stage), so
        # these boxes are already de-duplicated. We re-run cv2.dnn.NMSBoxes
        # anyway to cap the result at MAX_DETECTIONS deterministically and to
        # stay correct for any export whose graph lacks an in-graph NMS stage.
        cv_boxes = np.stack(
            [
                boxes[:, 0],
                boxes[:, 1],
...
def doctor() -> int:
    ...
    print(
        "== "
        + ("ALL CHECKS PASSED" if ok else "CHECKS FAILED — see fixes above")
        + " =="
    )
