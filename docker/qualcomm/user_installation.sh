...
# The reset must complete before Docker starts any container that opens
# /dev/fastrpc-*; otherwise the bounce can race a container holding fastrpc
# fds and data-abort the kernel (hence Before=docker.service below).
...
After=local-fs.target
Before=cdsprpcd.service docker.service
